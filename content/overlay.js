...
function bannerSignature(settings) {
  const enabled = settings.blockBannerEnabled !== false && Number(settings.blockBannerDensity) > 0;
  return [
    enabled ? "1" : "0",
...
// 横幅只在文案/开关/密度/速度/色相变化时重建，纯换主题不动它。
function applyBanners(host, root, settings, signature) {
  ...
  if (settings.blockBannerEnabled === false || !(density > 0)) return;
...
function applyOverlayTheme(host, settings) {
  const tokens = buildOverlayTokens(settings);
  for (const [name, value] of Object.entries(tokens)) {
    host.style.setProperty(name, value, "important");
  }
  host.style.setProperty("background", tokens["--sg-scrim"], "important");
}
...
function createOverlay() {
  ...
  return { host, root, style, panel, badge, title, reason, encourage, videoInfo, actions, homeBtn, optBtn };
}
...
function ensureOverlay(settings) {
  ...
  applyOverlayTheme(overlayRefs.host, settings);

  const signature = bannerSignature(settings);
  if (overlayRefs.host.dataset.sgBanner !== signature) {
    applyBanners(overlayRefs.host, overlayRefs.root, settings, signature);
  }
