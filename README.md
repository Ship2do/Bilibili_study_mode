...
- 主题：明暗（亮色 / 暗色 / 跟随系统）+ 六种主题色预设
- 拦截界面：是否显示视频信息、横幅字样 / 密度 / 颜色（七种预设，各分亮暗两版）
- 横幅预览与真正的拦截界面共用 `shared/banner.js` 的同一份渲染代码，只是整体等比缩小
- 横幅是静止的：拦截界面本身不滚动，所以没有「滚动速度」这种设定
- 三种拦截呈现方式：
...
首次安装会自动打开六步引导：选判定模式 → 配关键词（强模式下提供六个学科预设包
一键填充）或填 AI 接口 → 外观与横幅 → 密码锁 → 用刚配的规则实地试跑三个示例视频。
